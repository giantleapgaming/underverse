...
export function godownTransport(network: NetworkLayer, phaser: PhaserLayer) {
  const {
    world,
  ...
  defineComponentSystem(world, ShowAnimation, () => {
    ...
    if (
      ...
      const object = objectPool.get("transport", "Sprite");
      ...
      object.setComponent({
        ...
        once: (gameObject) => {
          gameObject.setTexture(missileSprite.assetKey, `transport-ore-${amount}-product.png`);
