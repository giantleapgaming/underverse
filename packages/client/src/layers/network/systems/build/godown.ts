...
export function buildGodownSystem(network: NetworkLayer, phaser: PhaserLayer) {
  ...
  defineComponentSystem(world, Build, () => {
    ...
    if (
      ...
      const textYellow = objectPool.get("build-godown-station-text-yellow", "Text");
      ...
      if (faction) {
        ...
        textYellow.setComponent({
          id: "build-godown-station-text-yellow",
          once: (gameObject) => {
      ...
      objectPool.remove("build-godown-station-text-yellow");
