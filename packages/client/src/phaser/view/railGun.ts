...
export function displayRailGunSystem(network: NetworkLayer, phaser: PhaserLayer) {
  const {
    scenes: {
      Main: {
        objectPool,
  ...
  defineSystem(
    ...
    ({ entity }) => {
      ...
      if (entityTypeNumber && +entityTypeNumber === Mapping.railGunShip.id) {
        ...
        if (+defence > 0 && +level) {
          ...
          for (let i = 1; i < 11; i++) {
            objectPool.remove(`railGun-health-${entity}-${i}`);
            objectPool.remove(`railGun-cargo-${entity}-${i}`);
          }
          const [boxes, color] = calculateHealthBar(level * 100, +defence);

          // creating the new health bar
          for (let i = 1; i < (boxes >= 10 ? 11 : boxes); i++) {
            const healthSprite = objectPool.get(`railGun-health-${entity}-${i}`, "Rectangle");
            healthSprite.setComponent({
              id: `railGun-health-${entity}-${i}`,
              once: (gameObject) => {
          ...
          for (let i = 1; i < +level + 1; i++) {
            const healthSprite = objectPool.get(`railGun-cargo-${entity}-${i}`, "Rectangle");
            healthSprite.setComponent({
              id: `railGun-cargo-${entity}-${i}`,
              once: (gameObject) => {
          ...
          const railGunShipObjectTop1Layer = objectPool.get(`railGun-top1-${entity}`, "Sprite");
          const railGunShipObjectTop2Layer = objectPool.get(`railGun-top2-${entity}`, "Sprite");
          ...
          objectPool.remove(`railGun-top1-${entity}`);
          objectPool.remove(`railGun-top2-${entity}`);
          objectPool.remove(`railGun-gray-${entity}`);
          objectPool.remove(`railGun-level-${entity}`);
          for (let i = 1; i < 11; i++) {
            objectPool.remove(`railGun-health-${entity}-${i}`);
          }
        }
