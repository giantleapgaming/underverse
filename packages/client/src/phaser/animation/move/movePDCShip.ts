...
export function movePDCShip(network: NetworkLayer, phaser: PhaserLayer) {
  const {
  ...
  defineSystem(world, [Has(ShowAnimation), Has(OwnedBy)], ({ entity }) => {
    ...
    if (
      ...
      const pdcShipObjectTop1Layer = objectPool.get(`pdc-top1-move-${entity}`, "Sprite");
      const pdcShipObjectGrayLayer = objectPool.get(`pdc-gray-move-${entity}`, "Sprite");
      const pdcShipObjectFlame = objectPool.get(`pdc-flame-move-${entity}`, "Sprite");
      ...
      pdcShipObjectTop1Layer.setComponent({
        ...
        once: (gameObject) => {
          ...
          phaserScene.add.tween({
            ...
            duration: 5_000,
            onComplete: () => {
              objectPool.remove(`pdc-top1-move-${entity}`);
      ...
      pdcShipObjectFlame.setComponent({
        id: `pdc-flame-move${entity}`,
        once: (gameObject) => {
          ...
          phaserScene.add.tween({
            ...
            duration: 5_000,
            onComplete: () => {
              objectPool.remove(`pdc-flame-move-${entity}`);
              destinationCircle.clear();
              sourceCircle.clear();
            },
          });
      ...
      pdcShipObjectGrayLayer.setComponent({
        id: `pdc-gray-move-${entity}`,
        once: (gameObject) => {
          gameObject.setScale(0.5);
          ...
          phaserScene.add.tween({
            ...
            duration: 5_000,
            onComplete: () => {
              objectPool.remove(`pdc-gray-move-${entity}`);
              destinationCircle.clear();
              sourceCircle.clear();
