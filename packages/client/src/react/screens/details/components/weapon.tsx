...
export const Weapon = ({ buyWeaponSystem }: { buyWeaponSystem: (kgs: number) => void }) => {
    ...
    <S.Details>
      <div>
          ...
          >
            {new Array(4).fill(0).map((_, i) => {
              return (
                ...
                >
                  <span style={{ marginLeft: "3px" }}>{(i + 1) * 100}</span>
                </S.Slanted>
              );
            })}
          </div>
        ...
        <S.Row style={{ justifyContent: "space-around", width: "100%" }}>
          <S.OtherDetails>COST {+selected > 0 ? convertPrice(+selected * 100) : "--:--"}</S.OtherDetails>
          <S.InlinePointer
            onClick={() => {
